...
def custom_torch_policy(meta_list):
    ...
    2. Allocates a torch.Tensor for each meta on CUDA
    ...
    for i, meta in enumerate(meta_list):
        ...
        # Allocate a torch tensor upon CUDA
...
async def tensor_op_func(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    """
    print(f"Server received tensors: type(a)={type(a)}, device(a)={a.device}")
    return a + b
...
async def main():
    ...
    # Initialize CUDA - this ensures context is created
    torch.cuda.init()
    # Trigger context creation if needed
    ...
    b = torch.ones((1024, 1024), dtype=torch.float32, device="cuda") * 2
    ...
    print(
        f"RNDV Result received: type={type(result_rndv)}, device={result_rndv.device}"
    )

    # Verify RNDV result
    assert isinstance(result_rndv, torch.Tensor), "Result should be a torch.Tensor"
    assert result_rndv.is_cuda, "Result of RNDV path should be on CUDA"
    ...
    print(
        f"Eager Result received: type={type(result_eager)}, device={result_eager.device}"
    )
